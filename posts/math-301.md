...
$$
k_1y_1(x)+k_2y_2(x)=0이면\ k_1=k_2=0 \tag{1}
$$
...
> 💡 **Key Takeaway**:
> - **2계 제차 미분 방정식 ($y'' + ay' + by = 0$)**: 특성 방정식의 해에 따라 두 실근($C_1e^{\lambda_1x} + C_2e^{\lambda_2x}$), 중근($(C_1+C_2x)e^{\lambda x}$), 또는 허근($e^{px}(C_1\cos(qx) + C_2\sin(qx))$) 형태의 기저(Basis) 선형 결합으로 해를 얻는다.
> - **2계 비제차 미분 방정식 ($y'' + ay' + by = r(x)$)**: 일반해는 동차 방정식의 해인 제차해($y_h$)와 비동차항을 만족하는 특수해($y_p$)의 합($y = y_h + y_p$)으로 도출되며, 특수해는 미정계수법 등으로 구한다.

## 상수 계수를 갖는 2계 비제차 미분 방정식
$y\prime\prime+ay\prime+by=r(x)$의 일반해 $y(x)$는 $y(x)=y_h(x)+y_p(x)$를 만족한다. (h : homogeneous (제차), p : particular (특수해)) 이때, 제차해는 제차 미분 방정식의 풀이법을 이용하고 특수해는 **미정계수법**으로 구한다. 미정계수법이란 $y_p$를 $r(x)$와 유사한 꼴에 적당히 계수를 붙인 것으로 가정한 다음, 주어진 방정식을 만족하도록 미정계수를 결정하는 방법이다.
