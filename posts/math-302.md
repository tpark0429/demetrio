...
$$
B = \begin{bmatrix} b_1 & b_2 & \cdots & b_n \end{bmatrix} \tag{5}
$$
...
> 💡 **Mathematical Insight**: 역행렬 $A^{-1}$의 $j$번째 열벡터 $b_j$는 **"단위 기저 입력 $e_j$에 대한 시스템 $A$의 응답"**이다. 임의의 입력 $b$에 대한 해 $x = A^{-1}b$는 각 기저 입력에 대한 응답들의 선형 결합으로 표현된다.
...
$$
\int_{-\infty}^{\infty} \delta(x-y) dx = 1 \tag{9}
$$
...
$$
L_x G(x,y) = \delta(x-y) \tag{15}
$$

여기서 첨자 $x$는 연산자 $L$이 변수 $x$에 대해서만 작용함을 의미한다.

> 💡 **Definition of Green's Function**: 그린 함수 $G(x,y)$는 **"연속 공간에서 디랙 델타 입력(점원, Point Source)에 대한 미분 연산자의 응답"**이다.
...
$$
G: \mathbb{R}^2 \rightarrow \mathbb{R} \tag{16}
$$
...
#### [예제 1] 1계 미분 방정식
$$
\frac{du}{dx} = f(x), \quad x \in [a, \infty) \tag{21}
$$
...
$$
\frac{\partial}{\partial x} G(x,y) = \delta(x-y) \tag{22}
$$

$x \neq y$인 영역에서는 $\delta(x-y) = 0$이므로, 그린 함수는 상수 함수 형태를 가진다.

$$
G(x,y) = \begin{cases} c_1, & x < y \\ c_2, & x > y \end{cases} \tag{23}
$$
...
$$
\int_{y-\epsilon}^{y+\epsilon} \frac{\partial}{\partial x} G(x,y) dx = G(y+\epsilon, y) - G(y-\epsilon, y) = \int_{y-\epsilon}^{y+\epsilon} \delta(x-y) dx = 1 \tag{24}
$$

$$
c_2 - c_1 = 1 \implies c_2 = 1 \tag{25}
$$
따라서 그린 함수는 다음과 같은 단위 계단 함수(Unit Step Function)의 형태를 띤다.

$$
G(x,y) = \begin{cases} 0, & x < y \\ 1, & x > y \end{cases} = H(x-y) \tag{26}
$$

**3. 최종 일반해 도출**
$$
u(x) = \int_a^\infty G(x,y) f(y) dy = \int_a^\infty H(x-y) f(y) dy = \int_a^x f(y) dy \tag{27}
$$

만약 $f(x) = x$라면 해는 다음과 같이 얻어진다.

$$
u(x) = \int_a^x y dy = \frac{1}{2}x^2 - \frac{1}{2}a^2 \tag{28}
$$
...
$$
\frac{d^2u}{dx^2} = f(x), \quad x \in [0, \pi] \tag{29}
$$
...
$$
\frac{\partial^2}{\partial x^2} G(x,y) = \delta(x-y) \tag{30}
$$

$x \neq y$인 영역에서는 두 번 미분해서 0이 되어야 하므로 1차 식 형태를 가진다.

$$
G(x,y) = \begin{cases} c_1 x + c_2, & x < y \\ c_3 x + c_4, & x > y \end{cases} \tag{31}
$$
...
따라서 다음과 같이 정리된다.
$$
G(x,y) = \begin{cases} c_1 x, & x < y \\ c_3 (x - \pi), & x > y \end{cases} \tag{32}
$$

**3. 연속성 및 도함수의 불연속성 검증**
그린 함수 자체는 $x=y$에서 연속이어야 하므로
$$
c_1 y = c_3 (y - \pi) \tag{33}
$$

1계 도함수는 $x=y$에서 1만큼의 점프(Discontinuity)를 가져야 하므로

$$
\left. \frac{\partial G}{\partial x} \right|_{x=y^+} - \left. \frac{\partial G}{\partial x} \right|_{x=y^-} = 1 \implies c_3 - c_1 = 1 \tag{34}
$$

두 식을 연립하여 $c_1, c_3$을 구하면 다음과 같다.
$$
c_1 = \frac{y}{\pi} - 1, \quad c_3 = \frac{y}{\pi} \tag{35}
$$

최종적인 그린 함수 $G(x,y)$는 다음과 같이 대칭적인 구조를 가진다.
$$
G(x,y) = \begin{cases} \left(\dfrac{y}{\pi} - 1\right)x, & x < y \\ \left(\dfrac{x}{\pi} - 1\right)y, & x > y \end{cases} \tag{36}
$$
...
$$
u(x) = \int_a^b G(x,y) f(y) dy \tag{37}
$$

Neural Operator는 **그린 함수 $G(x,y)$를 데이터로부터 학습 가능한 신경망 $\kappa_\theta(x,y)$로 대체**하여 근사한다.
$$
u(x) \approx \int_a^b \kappa_\theta(x,y) f(y) dy \tag{38}
$$
