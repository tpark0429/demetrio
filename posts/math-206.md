...
## Example: 몬테카를로 근사를 이용한 원주율($\pi$) 추정

가로세로의 길이가 1인 정사각형 영역 $[0, 1] \times [0, 1]$ 내에 무작위로 점을 찍어 원주율 $\pi$를 근사하는 예제이다.

### 1. 이론적 배경
- 한 변의 길이가 1인 정사각형의 넓이는 $1$이다.
- 반지름의 길이가 1인 원의 4분의 1(4분원)의 넓이는 $\frac{\pi}{4}$이다.
- 정사각형 영역 안에 무작위로 점 $(x, y)$를 균일하게 생성할 때, 해당 점이 4분원 내부($x^2 + y^2 \le 1$)에 위치할 확률은 $\frac{\pi}{4}$가 된다.
- 무작위 시행 횟수 $N$이 커질수록, 4분원 내부에 들어온 점의 개수 $N_{\text{in}}$의 비율은 기댓값 $\frac{\pi}{4}$에 수렴한다.
  $$ \frac{N_{\text{in}}}{N} \approx \frac{\pi}{4} \implies \pi \approx 4 \times \frac{N_{\text{in}}}{N} $$

### 2. 파이썬 구현 코드
아래 코드는 무작위 시행을 통해 점을 생성하고 원주율을 계산한 뒤 시각화하는 파이썬 스크립트이다.
...
if __name__ == '__main__':
    N = 10000  # 무작위 시행 횟수 정의
    x = np.random.random([N, 2])
    distance = np.sum(x ** 2.0, axis=1)
    in_out = distance <= 1.0
    
    # Pi 값은 전체 시행에서 원 안에 있는 점의 비율로 계산됨
    pi = np.sum(in_out) * 4 / N
    color = list(map(lambda val: 'red' if val else 'blue', in_out))  # 원 안/밖에 따른 색상 설정
...
### 3. 시뮬레이션 결과
시행 횟수 $N = 10000$으로 수행한 몬테카를로 시뮬레이션 결과 그래프는 다음과 같다. 파란색 점은 원 밖에 위치하는 점이며, 빨간색 점은 반지름 1 이하의 원 내부에 위치하는 점들을 나타낸다.
